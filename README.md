...
1. In Jenkins, create a **New Item** and select **Pipeline** as the project type.
2. Under the build trigger settings, configure the pipeline to trigger **from SCM**.
3. Select **Git** and paste the repository URL:
   ```
   https://github.com/AdityaMMantri/Kaggle-House-Price-Prediction.git
   ```
4. Change the branch from `master` to `main`.
5. Click **Save**, then click **Build Now** to trigger the pipeline.
6. Once the build completes, open the following in your browser:
   - Streamlit Dashboard: `http://localhost:8501`
   - Prometheus Metrics: `http://localhost:8000`
7. Copy the `prometheus.yml` file from the repository and paste it into your local Prometheus installation directory, replacing the existing configuration file.
8. Navigate to your Prometheus installation directory and run the following command:
   ```bash
   ./prometheus.exe --config.file=prometheus.yml
   ```
9. Open `http://localhost:9090/targets` — you should see **3 endpoints** with status **UP**.

---

#### Method 2: Using Docker

1. Open **Docker Desktop** and ensure the Docker daemon is running.
2. Navigate to the cloned repository folder and open the `Kaggle-House-Price-Prediction` directory.
3. Open a command prompt in that directory.
4. Build the Docker image by running:
   ```bash
   docker build -t streamlit_final_check .
   ```
5. Start the container by running:
   ```bash
   docker run -d --name house-price-container2 -p 8502:8501 -p 8000:8000 streamlit_final_check
   ```
6. Open the Streamlit dashboard in your browser at:
   ```
   http://localhost:8502
   ```
7. Copy the `prometheus.yml` file from the repository and paste it into your local Prometheus installation directory, replacing the existing configuration file.
...
1. Navigate to the cloned repository folder and open the `Kaggle-House-Price-Prediction` directory.
...
4. Launch the Streamlit application:
   ...
   ```
5. Open the Streamlit dashboard in your browser at:
   ```
   http://localhost:8501
   ```
6. Copy the `prometheus.yml` file from the repository and paste it into your local Prometheus installation directory, replacing the existing configuration file.
7. Navigate to your Prometheus installation directory and run the following command:
   ```bash
   ./prometheus.exe --config.file=prometheus.yml
   ```
8. Open `http://localhost:9090/targets` — you should see **3 endpoints** with status **UP**.
